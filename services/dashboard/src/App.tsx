...
const dateTimeWithDay = (day: CalendarDate, existing: string, endOfDay = false) => day.toString() + "T" + (existing.slice(11) || (endOfDay ? "23:59" : "00:00"));
...
function DatePill({ label, value, onChange, minimum }: { label: string; value: string; onChange: (value: string) => void; minimum?: CalendarDate }) {
  const [open, setOpen] = useState(false);
  const selected = parseDate(dayFromDateTime(value));
  const maxDate = today(getLocalTimeZone());
  const dialogLabel = label + " date";
  return <div className="date-field"><button type="button" className="filter-pill date-trigger" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen((current) => !current)}><span>{label}</span><strong>{formatDay(value)}</strong><CalendarIcon /></button>
    {open && <div className="calendar-popover" role="dialog" aria-label={dialogLabel}><Calendar aria-label={dialogLabel} value={selected} minValue={minimum} maxValue={maxDate} onChange={(date) => { if (!date) return; onChange(dateTimeWithDay(date, value, label === "To")); setOpen(false); }} className="date-calendar">
      <Calendar.Header className="calendar-header"><Calendar.NavButton slot="previous" aria-label="Previous month">‹</Calendar.NavButton><Calendar.Heading /><Calendar.NavButton slot="next" aria-label="Next month">›</Calendar.NavButton></Calendar.Header>
      <Calendar.Grid className="calendar-grid"><Calendar.GridHeader>{(day) => <Calendar.HeaderCell>{day}</Calendar.HeaderCell>}</Calendar.GridHeader><Calendar.GridBody>{(date) => <Calendar.Cell date={date} />}</Calendar.GridBody></Calendar.Grid>
    </Calendar></div>}
  </div>;
}
...
function Trend({ data }: { data: DashboardData["rollups"] }) {
  ...
  return <div className="trend-wrap"><svg className="trend" viewBox={[0, 0, width, height].join(" ")} role="img" aria-label="Usage trend">
...
function App() {
  ...
  const [filterOptions, setFilterOptions] = useState<FilterOptions>({ deviceIds: [], apps: [] });
  ...
  const fromDay = parseDate(dayFromDateTime(filters.from));
  ...
  function changeFrom(value: string) { setFilters((current) => ({ ...current, from: value, to: dayFromDateTime(value) > dayFromDateTime(current.to) ? dateTimeWithDay(parseDate(dayFromDateTime(value)), current.to, true) : current.to })); }
  ...
  return <div className="shell">
    ...
    <main id="top"><header className="topbar"><div><p className="eyebrow">Phone Behavior Analytics</p><h1>Usage overview</h1></div><div className="live"><i /> Live data <button type="button" className={"theme-toggle " + (sparkle ? "sparkling" : "")} onClick={toggleTheme} aria-label={"Switch to " + (theme === "dark" ? "light" : "dark") + " mode"}><span className="theme-sun"><SunIcon /></span><span className="theme-moon"><MoonIcon /></span>{[0, 1, 2, 3, 4, 5].map((star) => <em key={star} className={"spark star-" + star}>✦</em>)}</button><b>CY</b></div></header>
      <section className="filters" aria-labelledby="filter-title"><div className="filter-intro"><p className="eyebrow">Explore activity</p><h2 id="filter-title">Refine your view</h2><p>Compare usage patterns, sessions, and processing anomalies.</p></div><form onSubmit={submit}>
        ...
        <DatePill label="From" value={filters.from} onChange={changeFrom} />
        <DatePill label="To" value={filters.to} minimum={fromDay} onChange={(value) => update("to", value)} />
        <label className="filter-pill"><span>Group by</span><select value={filters.granularity} onChange={(event) => update("granularity", event.target.value as Granularity)}><option value="MINUTE">Minute</option><option value="HOUR">Hour</option><option value="DAY">Day</option></select></label>
        <button className="refresh" disabled={loading}>{loading ? "Refreshing…" : "Refresh data"}<span>→</span></button>
      </form></section>
