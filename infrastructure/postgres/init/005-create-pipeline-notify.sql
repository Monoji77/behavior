...
--
-- Function bodies use the named $fn$ dollar-quote tag rather than the bare
-- double-dollar form: the migration Job runs this from a Kubernetes container
-- command, where Kubernetes collapses a double dollar sign into a single one.
CREATE OR REPLACE FUNCTION notify_pipeline_open() RETURNS trigger AS $fn$
BEGIN
...
CREATE OR REPLACE FUNCTION notify_pipeline_session() RETURNS trigger AS $fn$
BEGIN
